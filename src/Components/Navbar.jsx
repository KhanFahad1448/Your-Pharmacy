...
import { ShoppingCart, User, Settings, ChevronDown, Zap, Menu, X } from "lucide-react";
    ...
    <header className="bg-white shadow-sm w-full">
      ...
      {menuOpen && (
        <div className="md:hidden bg-white px-6 py-4 border-b space-y-3">
          ...
          <Link to="/plus" className="block hover:text-blue-600">PLUS</Link>
      ...
      <nav className="hidden md:flex justify-center space-x-8 text-gray-700 text-base py-3 bg-white border-t">
        ...
        <Link to="/healthcare" className="hover:text-blue-600 flex items-center">Healthcare <ChevronDown className="w-4 h-4 ml-1" /></Link>
        <Link to="/lab-tests" className="hover:text-blue-600 flex items-center">Lab Tests <ChevronDown className="w-4 h-4 ml-1" /></Link>
        <Link to="/plus" className="hover:text-blue-600">PLUS</Link>
