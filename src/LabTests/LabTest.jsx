      ...
      <div className="relative z-10 text-center text-white">
        ...
        <div className="flex flex-wrap justify-center gap-3 sm:gap-4 mb-8 sm:mb-10">
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border border-white/60 transition-all duration-300 text-sm sm:text-base ${
                categoryFilter === cat
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
          {filteredTests.map((test) => (
            <div
              key={test.id}
              className="bg-white/20 p-3 sm:p-4 md:p-5 rounded-3xl shadow-md backdrop-blur-sm flex flex-col justify-between transition-all duration-300 hover:scale-105 hover:shadow-xl h-full min-h-[300px]"
            >
              <div>
                <h2 className="text-lg sm:text-xl font-semibold mb-1 sm:mb-2">{test.name}</h2>
                <p className="text-xs sm:text-sm md:text-sm mb-2 sm:mb-3 text-white/90 min-h-[48px] line-clamp-3">
                  {test.description}
                </p>

                <div className="flex items-center justify-between mb-2 sm:mb-3">
                  <span className="text-base sm:text-lg md:text-lg font-bold">
              ...
              <button
                ...
                className={`w-full mt-auto py-2 sm:py-2.5 rounded-xl font-semibold flex justify-center items-center gap-2 transition-all duration-300 text-sm sm:text-base ${
                  bookedTests[test.id]
              ...
              >
                {bookedTests[test.id] ? (
                  <>
                    <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5" /> Booked
                  </>
                ) : (
                  <>
                    <Stethoscope className="w-4 h-4 sm:w-5 sm:h-5" /> Book Now
                  </>
                )}
              </button>
            </div>
